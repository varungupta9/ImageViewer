...
//in-line css styling for components
const styles = theme =>({
    gridContainer: {
        width: "90%",
...
componentDidMount() //enables connection with API
{
    if (this.state.loggedIn) {
        let xhr = new XMLHttpRequest();
        let that = this;
        xhr.addEventListener("readystatechange", function () { // validates the connection
        ...
        xhrPost.addEventListener("readystatechange", function () {
            if (this.readyState === 4) {
                ...
                responseData.forEach(image => {
                    ...
                    date = new Date(date * 1000); //converts the created date in dd/mm/yyyy HH:MM:SS format
                ...
                that.setState({
                    ...
                    commentText: ''
                })  //set the state for posted images
            }
        });
...
likeHandler=(image) => //Function called once the like icon is clicked
{
    if (image.userliked) {
        if (image.likes.count > 0) {
            image.likes.count--; //decreases the count if like icon clicked twice
        }
        else {
            image.likes.count = 0;
        }
        ...
        image.likes.count++; //increases the like counter
    }
    image.userliked = !image.userliked;
    this.setState({ ...this.state }); //set the current state
}
...
searchItemHanlder = (search) => //Search the images with user input keywords
{
